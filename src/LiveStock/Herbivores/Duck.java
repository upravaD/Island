...
public class Duck extends Herbivores {
    ...
    public void eat(List<Object> list) { //Параметры: список ячейки массива island
        ...
        if (list.contains(Plants.plant.getPlantIcon())) { // Если список содержит plant
            ...
            setFoodSaturation(getFoodSaturation() + Plants.plant.weight/64); // Увеличиваем значение насыщения foodSaturation
            ...
            System.out.println(getClass().getSimpleName() + " eat " + StatisticData.plantEatCount + " times");
            move(list); // duck двигается дальше
        }
        else { //Если список не содержит plant

            setFoodSaturation(getFoodSaturation() - Plants.plant.weight/64); // Уменьшаем значение насыщения foodSaturation
            if (getFoodSaturation() > -0.001)
    ...
    public void multiply() {
        ...
        CellPosition.changeCell(Main.factory.createAnimal(AnimalType.DUCK.ordinal()).getIcon(), getCurrentPosition()); // Создаем duck через AnimalFactory
