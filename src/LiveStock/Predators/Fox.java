...
public class Fox extends Predator {
    ...
    /**
     * Алгоритм getIcon():
     * метод возвращает изображение обьекта fox
     */
    ...
    public void eat(List<Object> list) { //Параметры: список ячейки массива island
        ...
        if (list.contains(Caterpillar.caterpillar.getIcon()) && Main.random.nextInt(101) < 40) { // Если list содержит обьект Herbivores то с определленой вероятностью...

            list.remove(Caterpillar.caterpillar.getIcon()); // Удаляем animal из списка list
            setFoodSaturation(getFoodSaturation() + Caterpillar.caterpillar.getWeight()); // Увеличиваем значение насыщения foodSaturation
            ...
            if (getFoodSaturation() > getMaxFoodSaturation()) { // Если значение foodSaturation больше максимального
                if (Main.random.nextBoolean()) {
                    multiply(); // Создаем еще один обьект fox
            ...
            move(list); //fox двигается дальше
        }
        else if (list.contains(Rabbit.rabbit.getIcon()) && Main.random.nextInt(101) < 70) { // Если list содержит обьект Herbivores то с определленой вероятностью
        ...
        else if (list.contains(Duck.duck.getIcon()) && Main.random.nextInt(101) < 60) { // Если list содержит обьект Herbivores то с определленой вероятностью
            ...
            setFoodSaturation(getFoodSaturation() - 0.25); // Уменьшаем значение насыщения foodSaturation
            if (getFoodSaturation() > -0.0001)
            ...
            if (getFoodSaturation() < 0.0001) { // Если значение foodSaturation меньше 0.01
                list.remove(getIcon()); // Удаляем fox из списка list
                if (getFoodSaturation() > -0.0001) {
    ...
    /**
     * Алгоритм multiply():
     * метод создает fox в списке массива island
     */

    @Override
    public void multiply() {
        setCurrentPosition(Main.random.nextInt(Island.cellMaxSize)); // Сохраняем рандомное значение текущей позиции
        CellPosition.changeCell(Main.factory.createAnimal(AnimalType.FOX.ordinal()).getIcon(), getCurrentPosition()); // Создаем fox через AnimalFactory
